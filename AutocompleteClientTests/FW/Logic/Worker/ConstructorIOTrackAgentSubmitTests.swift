//
//  ConstructorIOTrackAgentSubmitTests.swift
//  Constructor.io
//
...
class ConstructorIOTrackAgentSubmitTests: XCTestCase {
    ...
    func testTrackAgentSubmit() {
        let builder = CIOBuilder(expectation: "Calling trackAgentSubmit should send a valid request", builder: http(200))
        ...
        self.constructor.trackAgentSubmit(intent: intent)
    ...
    func testTrackAgentSubmit_With400() {
        ...
        stub(regex("https://ac.cnstrc.com/v2/behavioral_action/assistant_submit?_dt=\(kRegexTimestamp)&c=\(kRegexVersion)&i=\(kRegexClientID)&key=\(kRegexAutocompleteKey)&s=\(kRegexSession)&\(TestConstants.defaultSegments)"), builder.create())
        self.constructor.trackAgentSubmit(intent: intent, completionHandler: { response in
    ...
    func testTrackAgentSubmit_With500() {
        let expectation = self.expectation(description: "Calling trackAgentSubmit with 500 should return internalServerError CIOError.")
        let builder = CIOBuilder(expectation: "Calling trackAgentSubmit should send a valid request", builder: http(500))
        stub(regex("https://ac.cnstrc.com/v2/behavioral_action/assistant_submit?_dt=\(kRegexTimestamp)&c=\(kRegexVersion)&i=\(kRegexClientID)&key=\(kRegexAutocompleteKey)&s=\(kRegexSession)&\(TestConstants.defaultSegments)"), builder.create())
        self.constructor.trackAgentSubmit(intent: intent, completionHandler: { response in
            if let cioError = response.error as? CIOError {
    ...
    func testTrackAgentSubmit_WithNoConnectivity() {
        let expectation = self.expectation(description: "Calling trackAgentSubmit with no connectivity should return noConnectivity CIOError.")
        stub(regex("https://ac.cnstrc.com/v2/behavioral_action/assistant_submit?_dt=\(kRegexTimestamp)&c=\(kRegexVersion)&i=\(kRegexClientID)&key=\(kRegexAutocompleteKey)&s=\(kRegexSession)&\(TestConstants.defaultSegments)"), noConnectivity())
        self.constructor.trackAgentSubmit(intent: intent, completionHandler: { response in
            if let cioError = response.error as? CIOError {
