...
class TrackQuizResultsLoadedRequestBuilderTests: XCTestCase {
    ...
    func testTrackQuizResultsLoadedBuilder() {
        let tracker = CIOTrackQuizResultsLoadedData(quizID: self.quizID, quizVersionID: self.quizVersionID, quizSessionID: self.quizSessionID, url: self.url, resultID: self.resultID, resultPage: self.resultPage, resultCount: self.resultCount, sectionName: self.sectionName, analyticsTags: self.analyticsTags)
