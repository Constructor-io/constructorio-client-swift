...
extension UIViewController {
    func fitViewInsideLayoutGuides(_ v: UIView) {
        if(v.superview == nil || v.superview != self) {
            v.removeFromSuperview()
            self.view.addSubview(v)
        }

        self.view.pinViewToLeft(v)
        self.view.pinViewToRight(v)
        self.view.addConstraint(NSLayoutConstraint(item: v, attribute: .top, relatedBy: .equal, toItem: self.topLayoutGuide, attribute: .bottom, multiplier: 1.0, constant: 0.0))
        self.view.addConstraint(NSLayoutConstraint(item: v, attribute: .bottom, relatedBy: .equal, toItem: self.bottomLayoutGuide, attribute: .top, multiplier: 1.0, constant: 0.0))

    }
}

extension UIView {
    func addViewToFit(_ v: UIView) {
        if(v.superview != self) {
            if v.superview != nil {
                v.removeFromSuperview()
            }
        ...
        self.addConstraint(NSLayoutConstraint(item: self, attribute: .bottom, relatedBy: .equal, toItem: v, attribute: .bottom, multiplier: 1.0, constant: 0.0))
