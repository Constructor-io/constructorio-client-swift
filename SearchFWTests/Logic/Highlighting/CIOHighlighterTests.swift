...
class CIOHighlighterTests: XCTestCase {
    ...
    func testHighlighter_NoMatches_NoHighlights() {
        ...
        XCTAssertEqual(result, NSAttributedString(string: "no matches in item!", attributes: [.font: provider.fontNormal]))
    }
    ...
    fileprivate func highlight(_ onEvenTokens: Bool, strings: [String]) -> NSAttributedString {
        let highlightedString = NSMutableAttributedString()
        var shouldHighlight = onEvenTokens
        for string in strings {
            guard shouldHighlight else {
                highlightedString.append(NSAttributedString(string: string, attributes: [.font: provider.fontNormal]))
                shouldHighlight = !shouldHighlight
                continue
            }

            highlightedString.append(NSAttributedString(string: string, attributes: [.font: provider.fontBold]))
            shouldHighlight = !shouldHighlight
        }
